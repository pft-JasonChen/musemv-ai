...
### A23. `/song/result` 的歌詞面板在「這首歌沒有歌詞」時是空的 —— 需要空狀態稿
...
**需要設計師給的:** 那個側欄在「沒有歌詞」時要長什麼樣子。目前的做法是**整塊不渲染**,
下面的 CTA 會往上移 —— 可用,但沒有人從設計角度看過。可能的方向是一句說明文字
(例如「這首歌沒有歌詞」)或是把該區換成別的資訊。

> 這一則的意義不只是補一張稿:它是「**用假資料填空版**」這個做法的一次實際代價。
> 空狀態沒畫,於是變成填假內容;假內容比空白更難發現,因為它看起來是對的。
...
### A25. `/mv/edit` 的頁首少了「N shots」與三個唯讀 chips —— `MVEditPage.css` 沒有可用的 class
...
**需要設計師給的:** 這三個元素在 `/mv/edit` 頁首的稿(含 class 命名),或是確認
**它們不該存在**,那樣我們就把 MV-P5-S1 對應的段落從規格拿掉。
