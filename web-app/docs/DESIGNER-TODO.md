...
（桌機右側歌詞面板 + Lyrics sheet）與 `/song/play`（Lyrics sheet）的每一行都會 seek。

**這不是稿裡的瑕疵，是稿裡沒有的控制項。** DP 的歌詞行是 `<p>`（純文字），
所以 `.lyrics-sheet__line` / `.song-result__lyrics-inline-line` 兩條規則裡
本來就沒有、也不需要任何互動狀態。我們把它換成 `<button>`（鍵盤與螢幕閱讀器要能操作，
理由同 `SeekBar`），並在 `src/styles/lyric-seek.css` 只做 reset ——
**目前外觀與 `<p>` 完全一樣，一個 pixel 都沒動**，所以視覺 baseline 不會變。
...
1. **每一首歌都可以點，不只有「有真實時間軸」的那一首。** prototype 裡只有
   `Neon Static`（產品負責人 2026-09-09 提供的真實 AI Song 結果）帶真實 LRC；其餘每一首的
   行時間是「把行數平均分配到歌長」推出來的。但**那個推估本來就一直在驅動 active 行的高亮**，
   所以點下去 seek 到同一個推估值，跟畫面上已經在宣稱的事情是一致的。反過來只讓一首能點，
   會讓使用者真正生成的每一首歌都還是踩到這個 bug。
2. **有 LRC 的時候，顯示的就是 LRC 的行，不是 `lyrics` 欄位的行。** 唱的行會拆、會重複、
