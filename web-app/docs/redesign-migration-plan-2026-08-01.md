...
> ## ⚠️ 決策已被 `redesign-migration-plan.md` 取代(2026-08-04)
>
> 本文是**研究與提案**,很多條目寫的是「建議,未拍板」。那些待決項已於 2026-08-04 逐條拍板,
> **結果在 `docs/redesign-migration-plan.md`,以它為準。**
>
> 本文仍然有效的部分:**§9(RD 契約面 C1–C8)** 與 **§10(Gate 制度 G1–G7)** —— 新計畫只記錄
> 對它們的修訂,不重寫。§8 的 S1–S19 與 §7 的 Q1–Q10 **仍是有用的清單,但判定看新計畫 §1**。
>
> 另外兩處本文已知過時:DP 的路徑(現在是 in-repo `designer-prototype/`)與
> 「DP 只覆蓋 9/20 route」(2026-08-04 的新包是 **15/20**)。
...
2. **`docs/CHANGELOG-RD.md`** —— **只**記錄契約面變更。CI 規則:
   _若 PR 動到 C1–C8 的檔案,但沒有動 `CHANGELOG-RD.md` → 直接 fail。_
...
2. `/mv/room`(DP 1,309 行,最大一支,含 5 個 sheet)
3. `/mv/thinking` + `/mv/storyboard` 7. `/mv/creating` + `/mv/result`
4. `/mv/edit` 9. `/song/create` + `/song/creating` + `/song/result`
