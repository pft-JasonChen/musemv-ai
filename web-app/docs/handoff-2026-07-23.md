...
> never edit token _values_ in `src/styles/tokens.css`; commit with explicit `git add <paths>`;
...
> ## ⚠️ Status as of 2026-08-06 — read this before using anything below
>
> **This document is a historical work-list, not a description of the current code.** §A and §B were
> implemented on 2026-07-23 and the file has not been maintained since; the whole designer-UI
> migration (Phase 3, 16 routes) happened after it and moved a lot of what it points at.
>
> - **The file pointers below are stale in places.** `MvDetail.tsx` no longer backs `/mv/result`
>   (it backs History's `CreationDialog` only), `SongDetail.tsx` no longer backs `/song/result`,
>   the `LEGACY_TAKE_TRAY_UI` flag it references is gone, and `/mv/room`'s six overlays are now DP
>   sheets. **`specs/areas/*.md` are the maintained as-built description — prefer them.**
> - **It is NOT archived** because §C–§F (Curation spec-only, backend-deferred, Phase 2,
>   still-open) are still live open items and this is where their reasoning is written down. Each
>   also appears in the matching area spec's §8; that is the copy that gets updated.
> - **The three "RD please confirm" reconciliations above are still unconfirmed.**
...
- **[GL-02 + EXP-02] Action-level auth gating.** Move from route-entry gating to gating at the action,
  matching App F22: Create MV / Create Song / **Like** / **Get Proof** trigger sign-in _at the action_.
...
- **[MV-06] / [HIST-04]** the publish→community _review pipeline_ (frontend confirm stays; pipeline deferred).
