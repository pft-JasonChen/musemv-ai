...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button role="switch" aria-checked={on} onClick={onToggle} className="relative h-6 w-10 rounded-full transition-colors" style={{ background: on ? "var(--accent)" : "var(--card-3)" }}>
...
function Row({ icon, title, sub, right, onClick }: { icon: React.ReactNode; title: string; sub?: string; right?: React.ReactNode; onClick?: () => void }) {
  return (
    <button onClick={onClick} disabled={!onClick} className="flex w-full items-center gap-3 px-1 py-3 text-left disabled:cursor-default">
  ...
  return (
    <div className="mx-auto max-w-[640px] px-4 py-8 sm:px-6">
      ...
      <div className="mt-4 divide-y" style={{ borderColor: "var(--border-3)" }}>
        ...
        <Row icon={<I d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0" />} title="Notifications" sub={notif ? "On" : "Off"} right={<Toggle on={notif} onToggle={() => setNotif((v) => !v)} />} />
        <Row icon={<I d="M4 5h16v14H4zM9 9h6v6H9z" />} title="Default Quality" sub={quality} right={<span style={{ color: "var(--text-3)" }}>›</span>} onClick={() => setQualOpen(true)} />
      ...
      <Modal open={qualOpen} onClose={() => setQualOpen(false)} title="Default Quality" maxWidth={360}>
        <div className="flex flex-col gap-2">
          {(["720p", "1080p"] as const).map((q) => (
            <button key={q} onClick={() => { setQuality(q); setQualOpen(false); }} className="flex items-center justify-between rounded-xl px-4 py-3 text-[14px] font-semibold" style={{ background: "var(--card-2)", border: `1.5px solid ${quality === q ? "var(--accent)" : "var(--border-2)"}` }}>
              {q} {quality === q && <span style={{ color: "var(--accent)" }}>✓</span>}
            </button>
          ))}
        </div>
      </Modal>
