...
REVIEW COPIES — read these, not the PNGs
    Every `shot()` also writes a downscaled WebP beside the PNG, under
    `specs/screenshots/_review/<name>.webp`. The PNG stays canonical for spec
    embedding; the WebP exists so an agent (or a review subagent) can LOOK at the
    screen without paying full price. Measured on real 1440x900 YCO captures:
    916 KB of PNG -> 91 KB of WebP, a 10.1x cut. Across the 474 screenshots this
    account has pulled into context, that is the single cheapest token saving
    available. Set YCO_REVIEW_COPIES=0 to disable; Pillow missing = silent skip.
...
# ── review-copy settings (see "REVIEW COPIES" in the module docstring) ───────
REVIEW_ENABLED = os.environ.get('YCO_REVIEW_COPIES', '1') != '0'
REVIEW_SUBDIR = '_review'
REVIEW_MAX_W = 1000   # 1440 -> 1000 keeps every layout defect legible
REVIEW_FMT = 'WEBP'   # beats JPEG ~1.8x on flat-colour UI captures
REVIEW_QUALITY = 80


def write_review_copy(png_path):
    """Downscaled WebP twin of `png_path`. Returns (path, orig_bytes, new_bytes)
    or None if disabled, Pillow is absent, or anything at all goes wrong — a
    review copy is an optimisation, never a reason to fail a capture run."""
    ...
    try:
        ...
        return None
    try:
        out_dir = os.path.join(os.path.dirname(png_path), REVIEW_SUBDIR)
        os.makedirs(out_dir, exist_ok=True)
        stem = os.path.splitext(os.path.basename(png_path))[0]
        out = os.path.join(out_dir, stem + '.' + REVIEW_FMT.lower())
        with Image.open(png_path) as im:
            im = im.convert('RGB')
            if im.width > REVIEW_MAX_W:
                im.thumbnail((REVIEW_MAX_W, 10 ** 6), Image.LANCZOS)
            im.save(out, REVIEW_FMT, quality=REVIEW_QUALITY, method=6)
        return out, os.path.getsize(png_path), os.path.getsize(out)
    except Exception:  # noqa: BLE001 — never break a capture over a thumbnail
        return None
...
class Capture:
    ...
    def __init__(self, feature_dir, base_path, viewport=(1440, 900),
        ...
        self.focus_map = {}
        self.errors = []
        self.review_dir = os.path.join(self.save_dir, REVIEW_SUBDIR)
        self._review_stats = [0, 0, 0]   # count, orig bytes, review bytes
        self._server = None
        self._pw = None
        self._browser = None
        self.page = None
    ...
    async def __aexit__(self, exc_type, exc, tb):
        ...
        n, orig, new = self._review_stats
        if n:
            print('REVIEW COPIES -> %s  (%d files, %.0f KB vs %.0f KB of PNG, %.1fx '
                  'cheaper to read)' % (self.review_dir, n, new / 1024, orig / 1024,
                                        orig / max(new, 1)))
            print('  ^ Read these for visual review. Never Read the full-size PNGs.')
        elif REVIEW_ENABLED:
            print('REVIEW COPIES: skipped (Pillow not importable?) — visual review '
                  'will cost ~10x more context than it needs to')
    ...
    def _record(self, name, path):
        """Log the capture and make its review copy. Called from every shot path."""
        r = write_review_copy(path)
        if r:
            _, orig, new = r
            self._review_stats[0] += 1
            self._review_stats[1] += orig
            self._review_stats[2] += new
            print('  +', name, '(review: %s/%s, %.0f%% smaller)'
                  % (REVIEW_SUBDIR, os.path.basename(r[0]), 100 * (1 - new / orig)))
        else:
            print('  +', name)
    ...
    async def shot(self, name, selector=None, clip=None):
        ...
        Side effect: also writes specs/screenshots/_review/<stem>.webp — the copy
        you should Read for visual review. See the module docstring."""
