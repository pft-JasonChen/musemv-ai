...
cfg = {
    ...
    # ── Data Contract (any payload the UI reads) ────────────────────────────────
  # Two renderers, chosen by what the contract IS:
  #   'schemas' — the payload's SHAPE. JSON sample + its keys documented under it
  #               (API-doc layout). Nesting and arrays survive; a flat table
  #               cannot show either. The builder highlights the JSON.
  #   'tables'  — per-key RENDER RULES. Five columns: key . allowed values .
  #               example . renders as . missing/null/unknown.
  # Use both when a spec needs both. Commit the real sample payload next to the
  # spec so lint_spec.py can measure coverage. Embedded payloads are redacted —
  # prompt values never ship.
  # 'data_contract': {
  #   'intro': 'What the web reads out of a <record>, and what each key becomes on screen.',
  #   'schemas': [{
  #       'caption': 'T1 &middot; Credit history record',
  #       'json': '{\n  "action": "pvImg2Vid_1080p",\n  "info": {"clientInfo": {}}\n}',
  #       'fields': [                       # (key, type, desc) — type optional
  #           ('action', 'string', 'Maps to MSR <code>action_name</code>.'),
  #           ('info.clientInfo', 'object', 'Free-form; only the keys below are read.'),
  #       ],
  #       'note': 'Optional footer under the field list.',
  #   }],
  #   'tables': [T_PAYLOAD, T_TYPES],
  #   'reverse': [('Template name on a tile', 'info.i18n[].data.title')],
  #   'payloads': [f'{FEAT}/specs/sample-payload.json'],
  # },
  # 'data_contract_ignore': ['createdTime', 'lastModified'],

  # ── Open Questions — undecided AND blocking. Settled answers go next to the
  # rule they govern, not here.
  # 'open_questions': [
  #   ('Q-01', 'What shows when <code>title</code> is absent?', 'T1 - template name', 'PM'),
  # ],

  # ── QA coverage — one row per plan.md acceptance criterion.
  # A criterion with no step needs a reason; validate() rejects a silent gap.
  # 'criteria': [
  #   ('S1.1', 'Sidebar shows the entry point.', ['P1-S1']),
  #   ('S1.4', 'Renders at every viewport.', [], 'Prototype Stage 3 validation'),
  # ],
