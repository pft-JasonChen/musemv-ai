...
- **AC-PROF-06** — WHEN Sign Out is invoked (from Settings — PROF-03 — or the account menu), THE SYSTEM SHALL clear auth and redirect Home. It SHALL NOT appear on `/profile`. Landing on Home after Sign Out SHALL NOT open the sign-in dialog automatically. _(`YMW260916P0020`, 2026-09-17: reported as a defect ("no sign-in dialog after sign out"), but the product owner agreed with RD's own comment on the ticket that an automatic popup right after leaving your account is not wanted — this line is the explicit record so it does not get re-reported.)_
