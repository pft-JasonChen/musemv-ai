...
| **MV-P1-S4** | Type / paste a description                                | Updates `n/2500` counter; blocks keyboard/paste past 2500                                                                                                                                                                                                                                                 | ⚠️ Hard 2500 cap is web-new. Shortcut: **Templates**. **`Enhance` was removed from this box 2026-08-25** — the engine has no refine mode for an MV description, so the control promised something the backend cannot do (product owner). Like `Ideas`, this is a subtraction FROM DP, so a drop will bring it back and the removal must be re-applied. **The "Ideas" button was removed 2026-08-06** — V1 ships no canned-sample fillers; a deliberate subtraction from DP, which still has one. Note: shortcut-fills are not length-capped (see AC-MV-03). |
...
- **AC-MV-14** — WHEN **Enhance** is invoked on the **visual style, scene prompt, or cover description**, THE SYSTEM SHALL replace that field with the value returned by `enhancePrompt` for the matching `kind`. _(Narrowed from four fields to three on 2026-08-25: the MV **description** no longer has an Enhance control — see MV-P1-S4.)_
...
- [ ] **MV-P6**: Enhance replaces each of the **3** remaining fields, and the MV description has **no** Enhance control (AC-14).
