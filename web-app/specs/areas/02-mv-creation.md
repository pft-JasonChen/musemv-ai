...
| **MV-P1-S4** | Type / paste a description                                | Updates `n/2500` counter; blocks keyboard/paste past 2500                                                                                                                                                                                                                                                 | ⚠️ Hard 2500 cap is web-new. Shortcuts: **Templates** and **Enhance**. **The "Ideas" button was removed 2026-08-06** — V1 ships no canned-sample fillers, a deliberate subtraction from DP, which still has one. **Enhance was removed 2026-08-25 (`3bdff87`) then RESTORED 2026-08-27** — the 2026-08-25 removal was product-owner-directed because the field had no supported backend API; RD confirmed one exists on 2026-08-26, so the removal no longer applies and the shortcut is back (same `EnhanceButton` / `enhancePrompt` round-trip as before, `kind="mv"`). Corrected 2026-08-27 twice in one day during the S2 storyboard-spec capture — first to record the removal, then to record its reversal; the S2 spec's screenshots and P2 scope note were captured **during the removal window** and are stale on this one point (see PLAN.md). Note: Templates' and Enhance's fills are not length-capped (see AC-MV-03). |
...
- **AC-MV-14** — WHEN **Enhance** is invoked on visual style, scene prompt, cover description, or the MV description, THE SYSTEM SHALL replace that field with the value returned by `enhancePrompt` for the matching `kind`. All four fields again — `/mv/room`'s Enhance button was removed 2026-08-25 (`3bdff87`, product owner, "not available in V1" — no supported backend API at the time) and **restored 2026-08-27** once RD confirmed the API exists. Corrected 2026-08-27 twice in one day during the S2 storyboard-spec capture: first to drop description (the removal), then to add it back (the reversal).
