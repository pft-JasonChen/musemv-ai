...
| **MV-P1-S4** | Type / paste a description                                | Updates `n/2500` counter; blocks keyboard/paste past 2500                                                                                                                                                                                                                                                 | ⚠️ Hard 2500 cap is web-new. Shortcut: **Templates** only. **The "Ideas" button was removed 2026-08-06, and Enhance was removed 2026-08-25** (`3bdff87`, product owner — "not available in V1") — V1 ships neither canned-sample fillers nor an Enhance shortcut on this field; both are deliberate subtractions from DP, which still has both. `EnhanceButton` itself and the `enhancePrompt` round-trip are untouched — this screen just no longer renders the button (`MvRoom.tsx`'s own header note). Corrected 2026-08-27 during the S2 storyboard-spec capture; this row previously still listed Enhance as a shortcut here. Note: Templates' fill is not length-capped (see AC-MV-03). |
...
- **AC-MV-14** — WHEN **Enhance** is invoked on visual style, scene prompt, or cover description, THE SYSTEM SHALL replace that field with the value returned by `enhancePrompt` for the matching `kind`. ⚠️ **The description field is no longer one of the four** — `/mv/room`'s Enhance button was removed 2026-08-25 (`3bdff87`, product owner, "not available in V1"), so Describe has no Enhance shortcut in this build; only Templates remains (MV-P1-S4). `enhancePrompt` itself is unchanged and still reachable from the other three surfaces. Corrected 2026-08-27 during the S2 storyboard-spec capture — this criterion previously named all four fields including description.
