...
- **AC-CR-09** — WHEN a Muse Pro plan is subscribed to in `SubscribeModal`, THE SYSTEM SHALL update the header credit count and expiry cadence to that plan (200 Weekly / 1,000 Weekly Pro / 2,000 Yearly). **There is no default selection** — each card carries its own Subscribe button. _(Corrected 2026-08-19: the "default selection SHALL be Weekly Pro" clause contradicted `CR-P2-S1` in this same file and has not matched the code since slice 3f removed the shared selection state.)_
