...
- **SONG-P1-S0 (side rail, 2026-08-06; source corrected 2026-09-09)** Same two-mode aside as `/mv/room` (area 02 MV-P1-S0): **"Trending Songs"** over `TOP_PICKS_SONGS` with a "See all" → `/explore/songs`, or **"My Creations"** over the user's own finished songs, no "See all", each row opening `/song/result?id=`. Requires `loggedIn` **and** at least one completed song — and since `YMW260902P0013` "the user's own finished songs" means `useMyCreations("song")`: the live session jobs **merged with the same seeded creations `/history` shows**, not the session jobs alone. See `AC-SONG-19`.
- **SONG-P1-S1** Arrive `/song/create` (**guest-reachable** — the route guard was removed 2026-08-12, the gate is on the Create Song button; see `SONG-E3` / `AC-AUTH-08`); **Custom** tab default, **Simple** second (product owner, `YMW260910P0021`, 2026-09-11 — was Simple-first); **Create Song** disabled until `describe` non-empty **while on Simple**. Hint "Describe your song to continue."
...
- **SONG-P3-S2** **Lyrics** → the shared `LyricsSheet` (synced highlight + mini player) — only when lyrics exist. In practice this means **Custom mode + non-instrumental + typed lyrics**; Simple mode never sets `lyrics`, so a Simple-mode result has no Lyrics sheet. Every line in the sheet, and in the desktop inline panel, **seeks to its own timestamp** on click (`AC-SONG-18`, 2026-09-09).
...
| ----------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SONG-E1** | Song job fails                                                              | `SongGenerationScreen`'s failure state: "Generation Failed" + **Back** (`/song/create`) only, "credits were not charged" — **no Retry** (product owner, `YMW260910P0020`, 2026-09-12; was Back + Retry). **`[fail]` in the Simple-mode `describe` triggers a mock failure at ~60% (`mock.ts:137`); `lyrics` does not — so a Custom-mode song cannot be failed via the UI.** Production trigger → `TBD-SONG-06`.                                                                                                                                                                                                                                    |
...
- **AC-SONG-01** — WHEN `/song/create` loads, THE SYSTEM SHALL default to **Custom** (tab order Custom, Simple — `YMW260910P0021`, 2026-09-11), enabled by default; on **Simple**, **Create Song** SHALL stay disabled until `describe.trim() !== ""`.
...
- **AC-SONG-18** _(new 2026-09-09, `YMW260903P0005`)_ — WHEN lyrics are shown on `/song/result` (the desktop inline panel **and** the Lyrics sheet), THE SYSTEM SHALL make each line an operable control that **seeks playback to that line's start time**, and SHALL highlight the line whose time has been reached.
  > **Where the times come from, because the two sources are not equivalent.**
  >
  > · **Real** per-line timing, WHERE the result carries it: `SongResult.lyricsLrc`, an LRC block (`[mm:ss.cc]line`) that the AI Song backend already returns as `timestamps.lyrics_lrc_timestamps`. Added to the contract for this criterion — C2 additive, `docs/CHANGELOG-RD.md` 2026-09-09.
  > · **Derived** timing OTHERWISE: the non-empty lines spread evenly across the track's duration. This is not new — it is what has always driven the highlight — and it is why click-to-seek is offered on every song rather than only on timed ones. The highlight already asserts a line↔time correspondence; seeking to the same estimate is consistent with what the screen shows, whereas withholding the control would leave the reported bug in place for every song a user actually generates. What real timing changes is **accuracy**, not whether the affordance exists.
  >
  > WHERE an LRC exists it is also what gets **displayed**: the sung line list can split, repeat or ad-lib where the written `lyrics` does not, and drops `[verse]`/`[chorus]` section tags. Aligning the two would mean inventing a mapping.
  >
  > 🔒 In the prototype exactly ONE song has real timing — the vendored `Neon Static` sample (product owner, 2026-09-09: audio, cover and LRC), reachable as a seeded creation (`h-neon-static`) and as a catalog song (`sp-neon-static`). It plays its own mp3 rather than one of the two shared demo tracks, because cues measured against one track land on the wrong beats of another.
  > The same rule applies to `/song/play`'s Lyrics sheet — see `AC-EXP-05`.
- **AC-SONG-19** _(new 2026-09-09, `YMW260902P0013`)_ — WHILE the user is signed in, `/song/create`'s side rail SHALL show **My Creations** over their own finished songs, and SHALL behave identically to `/mv/room`'s rail (area 02, `AC-MV-21`). WHEN they are signed out, or have no finished songs at all, it SHALL fall back to **Trending Songs** with "See all".
- **AC-SONG-21** _(new 2026-09-22, `YMW260902P0006`, product owner — NAB)_ — WHEN a Custom-mode song finishes and its lyrics are shown on `/song/result`, THE SYSTEM SHALL display **the lyric lines the generation engine returns**, which do **not** carry the section markers (`[intro]`, `[verse]`, `[chorus]`, `[bridge]`, `[outro]`) the user typed into the Lyrics field. The absence of those markers in the result is **expected behavior, not a defect**: the markers are an instruction to the engine about song structure, not text to be sung back, and the engine's output is generated lyric lines rather than an echo of the submitted text. THE SYSTEM SHALL NOT re-insert them.
  > **This prototype is the exception, and it is why the bug was filed.** Backend-less, it echoes the typed `lyrics` string straight into the panel, so the markers DO appear here — see the `SONG-P2-S11` capture in the Song Creation storyboard, which is a picture of the prototype, not of the product. The one place this repo behaves like the real thing is the vendored **Neon Static** sample, the only song with a real LRC: there the displayed lines come from the LRC, which splits, repeats and ad-libs, and carries no markers.
  >
  > Reported by `BARRY_KAO` 2026-09-02, proposed NAB by `SMITH_LEE` 2026-09-04 and by the PM 2026-09-09; `BARRY_KAO` asked on 2026-09-21 for the expected behavior to be written into the spec rather than only argued on the ticket. This criterion and the storyboard rule are that record. **Screenshots deliberately unchanged** (product owner, 2026-09-22): the captures are of the prototype and stay as they are — only the wording is corrected.

- **AC-SONG-20** _(new 2026-09-21, `YMW260911P0004`, product owner)_ — WHILE `/song/create`'s side rail is waiting on its data, THE SYSTEM SHALL render a **skeleton** of the list, identically to `/mv/room`'s rail. The full rule, the reason this rail is the one exception to the app's 3-dot loading UI, and the note that it is RD-implemented rather than mocked here, are written once in area 02 `AC-MV-22` / `MV-E10`.
  > **What the reported defect actually was.** Both rails already ran the same condition over the same source — the session-local `HistoryProvider`, which starts EMPTY. So a signed-in user saw "My Creations" on whichever create screen they had just generated something on, and "Trending" on the other; the reporter had made an MV and no song, and the two pages looked like they disagreed. The code was identical and the DATA was lopsided.
  >
  > The fix gives both rails the source `/history` has always had (`useMyCreations`): live session jobs **plus** the seeded creations. A `status: "done"` filter and a `source: "community"` exclusion apply — a rail titled "My Creations" must not claim a community song that merely appears in History (`h-whispers-past`), and a processing or failed row has no result screen to open.
  >
  > The "at least one finished song" guard **survives** for the signed-out case, and because the alternative was tried: DP's literal `isSignedIn` plus an empty-state card was shipped and reverted on 2026-08-07 — a "My Creations" heading over a blank card read as broken rather than empty.
  > **Known, unfixed, and now VISIBLE: the rail flashes the signed-out branch for a frame on a signed-in reload.** Every route is prerendered with `authStore.getServerSnapshot() === false`, so the served HTML carries "Trending Songs" and 7 community rows, and hydration swaps it. This is not new — it is `SHELL-E1`, the same pre-hydration swap the navbar's Login ↔ credit-pill has — but it used to be invisible here, because before this criterion a signed-in user with no session creations saw Trending too, so both states rendered the same thing. A backend-less prototype has no server-side auth to fix it with; recorded in area 01 §5 rather than worked around.
...
had not been touched since the 2026-08-12 repricing; tab default corrected 2026-09-11, `YMW260910P0021` — was Simple):** Custom default; Custom is Genre/Mood/Vocal
