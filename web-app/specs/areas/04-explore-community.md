...
- **Newly Released Songs** — `NEW_SONGS.slice(0,6)` in 2 columns (1 column below 768px); per row a
  **title** that navigates to `/song/play?id`, **album art** that starts `SongPlayBar` (the desktop
  ...
  Like (local), Share (`ShareDialog`), and **Create** → `requireLogin` → `/song/create` (blank —
  no longer seeds the compose form; removed 2026-09-17, `YMW260916P0021`).
...
- ❗ **The rail titled "Trending Music Videos" is fed by `NEW_MVS`, not `TRENDING_MVS`** — and that
  ...
  > **⚠️ The collision got a second instance, 2026-09-11.** `/explore/mvs`'s own primary section —
  > `TRENDING_MVS`, the one this bullet is about — is now ALSO titled "Trending Music Videos" on
  > desktop/tablet (was "Top Picks Music Videos"; §3.2 has the full history). So the product now
  > has **two different sections named "Trending Music Videos"**, on two different routes, fed by
  > two different arrays: Home's (`NEW_MVS`) and `/explore/mvs`'s (`TRENDING_MVS`). Same product
  > owner call as the first instance — not a defect, not a request to rename either — but worth
  > naming explicitly so QA doesn't file this pairing as a duplicate-content bug either.
...
- **`/explore/mvs`** (`MvExplore`): **two sections** (Top Picks ← `TRENDING_MVS`, Newly Released ← `NEW_MVS`), each a **justified-row gallery** at ≥1024px and a wrapping grid below; card → **navigates to `/watch?id=`** (2026-08-06 — it used to open `CommunityMvDialog` in place; DP's grid links at `/mv-detail?id=`, which is this app's `/watch`, so the dialog was a second rendering of a screen that already existed); Back → `DetailNavbar`'s `router.back()` with a fallback to `/`. Every card's creator avatar is `DEFAULT_CREATOR.avatar` ⚠️.
  ...
  > state. `/watch`'s mobile treatment (`.mv-detail--selected`) still hides both sections outright —
  > that half is unchanged.
  > **Top Picks row Next/Previous step, also settled 2026-09-11 (`YMW260909P0008`).** The
  > desktop-only `MvTopPicksRow` (≥1024px) originally stepped by exactly one card + gap per click;
  > QA/an automated agent test flagged the live build advancing further than that per click as a
  ...
  > **Two more same-day follow-ups, both product owner, 2026-09-11.** (1) "We forgot the tablet
  > version" — the row treatment above landed phone + desktop only; `MvGrid`'s tablet branch
  > (`!isDesktop`, 768–1023px) still fell through to the plain wrapping grid regardless of `asRow`.
  > Fixed the same way: tablet now renders the row too, at its own height
  > (`TABLET_ROW_HEIGHT` = 200, `MvGridSections.tsx` — no Figma frame for this width, picked as a
  > middle value between phone's 142 and desktop's 280). "Newly Released MV" is unaffected — it
  > keeps the wrapping grid at every non-desktop width, tablet included, exactly as before. (2) The
  > desktop/tablet title (`.section-header__title--desktop`, shown ≥768px) changed from "Top Picks
  > Music Videos" to **"Trending Music Videos"** — see §3.1's note on the resulting second instance
  > of that exact title existing on a different route/array pairing.
...
- `/watch` reads `?id` → `getCommunityMv(id) ?? NEW_MVS[0]`; stage sized to the item's own ratio
  ...
  (mvType + prompt only — `YMW260910P0008`, product owner, 2026-09-12, reaffirmed with a reason
  2026-09-22: **V1 does not carry the matched song or title into the handoff**) → `/mv/room`
  (area 02); a transport row with play/pause, a keyboard-operable `SeekBar`, mute
  and fullscreen; and **`MvGridSections` below the player** — the same two sections
  `/explore/mvs` renders.
  > **Why it is only two fields: music licensing.** The community MV's track is licensed, so
  > handing it to a new creation is a music-rights problem, not a scope preference. Carrying
  > official music into the composer is a **next-version** feature. (Product owner, 2026-09-22,
  > answering `BARRY_KAO`'s 2026-09-21 challenge to the reduced scope.)
  >
  > **The CTA is therefore DISABLED on arrival, and that is required behavior.**
  > `isComposeReady` is `song != null && description.trim().length > 0`, so a handoff with a
  > prompt but no song necessarily leaves **Create Music Video** disabled until the user picks a
  > song. "Prompt-only prefill" and "CTA enabled" cannot both be true — do not re-add a song to
  > light the button up.
  >
  > ✅ **Code now matches this row (2026-09-22).** `CommunityMvPlayer.tsx`'s `createMv()` had
  > seeded the matched song and title since 2026-08-05 (`cdba535c`); that prefill is removed.
  > Between 2026-09-12 and 2026-09-22 this file carried a ⚠️ recording the divergence as
  > known-and-accepted — it is resolved, not still open. Guarded by
  > `e2e/behaviour-regressions.spec.ts` → `YMW260910P0008`, which asserts both halves (prompt
  > present, CTA disabled) so neither can regress silently.
...
- **EXP-P1-S3** Tap a Trending/New MV card → `/watch?id`; a Top Picks/New Song's title/cover → `/song/result?id&from=home` at ≥768px, `/song/play?id` below it (`YMW260914P0002`); a New-Songs **Create** → `requireLogin` → `/song/create` (blank, not pre-filled — `YMW260916P0021`); Like/Share act locally.
...
- **EXP-P5-S2** **Create AI Song** → `/song/create`, blank — no longer pre-fills genre/mood/title/lyrics (removed 2026-09-17, `YMW260916P0021`).
...
- **AC-EXP-01** — WHEN `/` loads, THE SYSTEM SHALL render the hero, the tool selector, and the three seed rails (Trending Music Videos, Top Picks Songs, Newly Released Songs) in seed order — and SHALL mount the phone treatment of the hero and tool selector below 768px and the desktop treatment at or above it.
  ...
  > detail.
- **AC-EXP-02** — WHEN a hero CTA, a tool-selector card, or a New-Songs **Create** is tapped, THE SYSTEM SHALL run `requireLogin` and, on success, navigate to the create flow. The create flow opens blank; it does NOT pre-fill the tapped song's details (reversed 2026-09-17, `YMW260916P0021` — an earlier version of this line required pre-filling genre/mood/title/lyrics, which the product owner decided was more than V1 wants).
- **AC-EXP-03** — WHEN an MV card is tapped **anywhere** (Home rails or `/explore/mvs`), THE SYSTEM SHALL navigate to `/watch?id`. WHEN a song card's **title/cover** is tapped, on **either Home rail** (Top Picks Songs or Newly Released Songs) or **on `/explore/songs`**, THE SYSTEM SHALL, below 768px, open the full-screen player at `/song/play?id`; and at 768px and above, seed `SongFlow` and navigate to `/song/result?id&from=…` (`from=home` from a Home rail, `from=song-detail` from `/explore/songs`). WHEN a row's **album art/Play control** is tapped at 768px and above, THE SYSTEM SHALL start a preview in the persistent bottom bar **without navigating**. _(Corrected 2026-09-17, `YMW260914P0002`: this line previously said Home always goes straight to `/song/play?id` — that was already stale for `NewSongsSection` (fixed 2026-08-13) and was live-tested wrong for `TopPicksSection`, which had never received that fix; both Home rails now match `/explore/songs`.)_
...
- [ ] **EXP-P3**: song lists → player; Create → /song/create, blank, not pre-filled (AC-03); tab bar reads All · Pop · Hip-Hop · R&B · Rock · Jazz · Electronic · Rap · Classical · Country, every tab non-empty.
...
- [ ] **EXP-P5**: real `<audio>` progress; Prev/Next cycle; Lyrics; Create AI Song opens blank, not pre-filled (AC-05).
...
flowchart TD
  ...
  Play -->|Create AI Song| Song["/song/create (area 03, blank)"]
  Creator -->|row| Watch
  Creator -->|row| Play
```
