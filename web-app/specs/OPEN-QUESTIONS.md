...
> **更新到 2026-09-02 現況。** 這份檔案先前停在 2026-08-12(含 08-19/08-20 補丁),沒跟上 09-01 的
> 裁示,PM 拿舊版給 RD 會誤導。這次更新關掉了一項已結案卻還列著待辦的項目(`TBD-EXP-03`)、
> 修正了兩處已過期的「Curation PRD 至今未讀」敘述(PRD 已於 09-01 讀完,但這**沒有**解掉
> `TBD-EXP-11` 契約缺口——見下方),並補上一個**遺漏的第三個契約缺口**
> (`TBD-PROF-07`——Send Feedback 打哪個端點,見下方 🔧 後端實作表)。完整的交接盤點(含
> storyboard spec 狀態、gate 結果)見 `docs/RD-HANDOFF-READINESS-2026-09-02.md`,那份文件的
> 結論優先於本檔案下方 §🚦 這段 08-12 的舊版口吻。

---

## 🚦 可以交接給 RD 了嗎?(2026-08-12 盤點;下方 TBD 狀態已更新到 09-02)
...
### 🔴 交接前必須補的 —— 這三項是硬阻礙(2026-09-02 更新:補上第 3 項,先前漏列)

| #     | 缺什麼                                                                                             | 為什麼擋住 RD                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | **整條 Explore / Community 的 API 契約**(`TBD-GL-05` / `TBD-EXP-01` / `TBD-EXP-07` / `TODO.md` #1) | `MuseApi` **完全沒有** community endpoint(feed / detail / like / share / publish / creator)。**17 個元件、橫跨 9 條 route** 目前跑在 `src/lib/mv/community.ts` 的寫死種子上。schema(`CommunityMv` / `CommunitySong` / `CommunityCreator`)有了,消費它們的介面沒有。⚠️ **來源 PRD 已於 2026-09-01 讀完**(`YouCam_Muse_Explore_Curation_PRD - V2.pdf`,見 S8 storyboard 的 Phase 0 gate),但讀完**沒有解掉這個阻礙** —— PRD 只定義了排名/資格/審核的**語意**,沒有 endpoint、沒有欄位名、沒有 payload 形狀,`TBD-EXP-11` 依然是「未出現的 section」本身。 |
...
| **3** | **Send Feedback 打哪個端點**(`TBD-PROF-07`,2026-09-02 補列)                                        | Muse 與 YCO 的 CSB **不是同一套 API**(產品負責人 2026-08-27 的判讀)。若屬實,`areas/06` 從 CSB/T3 文件繼承的四個 `questionTypeId`、`prodVerId 504`、10 MB 附件上限**全是暫定值**(附件上限現已改為 5 MB,這是需求不是偏差)。RD 需指認真實端點並確認是否要 `title`(Subject 欄位已拿掉,若後端要求 title,決定權回到產品負責人,不能自己猜)。**擋住每一次真的送出。**                                                                                                                                                                                      |
...
| ~~**5**~~ | ✅ **2026-08-12 結案** — `DEFAULT_CREDITS = 10`。⚠️ 10 **做不出任何 MV**(最便宜 **105** —— 2026-09-03 更正,原本寫 220,那是已於 2026-08-19 刪除的 `COST_STORYBOARD` 20 + `COST_RENDER` 200;改成依秒計價後已無單一 MV 價格,下限是最便宜的組合:30 秒 storytelling / Standard,`45 + 2×30`。storyboard 路線 107,預設 compose(singing/Standard)30 秒是 195。已由 `contract.surface.test.ts` 鎖住),免費帳號生一首人聲歌就撞到付費牆 —— 這是刻意的漏斗。demo 另有 `NEXT_PUBLIC_DEMO_CREDITS` 環境變數可覆寫。                                                                                                                                                                                                                                    |
...
| **TBD-EXP-11** 🔴  | **Explore / 契約** | **交接阻礙。** `MuseApi` 完全沒有 community endpoint(feed / detail / like / share / publish / creator),但 17 個元件、9 條 route 跑在 `lib/mv/community.ts` 的寫死種子上。schema 已存在(`CommunityMv` / `CommunitySong` / `CommunityCreator`),消費它們的介面不存在。⚠️ **來源 PRD 已於 2026-09-01 讀完**,但只定義了排名語意,沒有 endpoint/欄位/payload —— 阻礙依然成立。見 `TODO.md` #1。 |
| **TBD-PROF-07** 🔴 | **Profile / 契約** | **交接阻礙,2026-09-02 補列(先前漏了)。** Send Feedback 打哪個端點未定 —— 產品負責人 2026-08-27 判讀 Muse 可能不在 YCO 的 CSB 上,若是,四個 `questionTypeId`、`prodVerId 504` 全是暫定值,且需確認後端是否要 `title`(欄位已拿掉)。詳見 `areas/06` §8。                                                                                                                                      |
...
## ⏳ 仍待議 / 待設計 — 這 5 項還需要你或設計師後續拍板(2026-09-02 更新計數)
...
| ~~TBD-EXP-03~~   | Explore | ✅ **2026-09-01 結案 — 維持現狀,不用切換。** 播放器比例由內容自動決定(`AC-EXP-04`),web 沒有、也不需要 9:16↔3:4 手動切換;上滑看下一支的手勢**保留**(`CommunityMvPlayer.tsx` 已實作並確認,見 `areas/04` §8)。`DESIGNER-TODO` A26 是過期文件,以程式碼為準。 |
